...
def wrap_expectations(cmd):
    script = create_script(cmd)
    remote_script = '/tmp/fexpect_' + shortuuid.uuid()
    ...
    fabric.api.put(StringIO(script), remote_script)
    wrapped_cmd = 'python ' + remote_script
    return wrapped_cmd, remote_script


def wrap_expectations_local(cmd):
    script = create_script(cmd)
    remote_script = '/tmp/fexpect_' + shortuuid.uuid()
    with open(remote_script, 'w') as filehandle:
...
def create_script(cmd):
    ...
    s += spwnTem.format(
        ...
        cmd=cmd,
        to=to,
        shellPrefix=('' if useShell.startswith('/') else '/bin/'),
    )
